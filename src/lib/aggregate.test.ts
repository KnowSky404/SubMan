...
describe("AnyTLS support", () => {
	it("infers anytls from anytls URI scheme", () => {
		expect(
			inferNodeTypeFromRaw("anytls://password@example.com:443#AnyTLS"),
		).toBe("anytls");
	});

	it("splits adjacent AnyTLS and VLESS nodes", () => {
		const content =
			"anytls://password@example.com:443#AnyTLSvless://uuid@example.com:443#VLESS";
	...
	it("keeps AnyTLS lines during subscription normalization", () => {
		...
		).toBe("anytls://password@example.com:443?sni=example.com#AnyTLS");
	});
...
describe("single node protocol inference", () => {
	...
	it("keeps the selected protocol when the URI protocol is unknown", () => {
		expect(
			inferNodeTypeFromDraft("custom://example.com:443#Custom", "ss"),
		).toBe("ss");
	});
});
