...
function committedResult(mutation: JsonRecord): JsonRecord {
	const payload = mutation.payload as JsonRecord;
	let data = structuredClone(EMPTY_DATA) as unknown as JsonRecord;
	...
	if (mutation.kind === "workspace.reconcile") {
		...
		data = {
			...
			nodes: [{ ...node, updatedAt: mutation.createdAt }],
		};
...
async function fulfillCommitted(
	...
	await route.fulfill({ status: 200, json: committedResult(mutation) });
}
...
test("rolls back the visible node when the IndexedDB transaction fails", async ({
	...
	await expect(page.getByText(/Workspace change was not saved/)).toBeVisible();
